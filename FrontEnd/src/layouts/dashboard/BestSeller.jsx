...
const BestSeller = () => {
    ...
    const handleSearch = () => {
        if (validateDates()) {
            setFilterValues({ ...filterValues, minDate: formatDate(minDate), maxDate: formatDate(maxDate), brandId: brandId, shoeTypeId: shoeTypeId })
        }
    };
        ...
        <div className='space-y-10'>
            <div>
                <div className='flex gap-4 items-center bg-[#ece9e9] p-3 rounded-md'>
                    ...
                    <input value={minDate} onChange={(e)=> setMinDate(e.target.value)} type="date" className='rounded-md' />
                    <label htmlFor="to">Ngày kết thúc</label>
                    <input value={maxDate} onChange={(e)=> setMaxDate(e.target.value)} type="date" className='rounded-md' />

                    <div className='flex items-center gap-2'>
                        <label >Thương hiệu</label>
                        <select className='rounded-md' value={brandId} onChange={(e)=> setBrandId(e.target.value)} >
                    ...
                    <div className='flex items-center gap-2'>
                        <label>Loại</label>
                        <select className='rounded-md' value={shoeTypeId} onChange={(e) => setShoeTypeId(e.target.value)}>
                            <option value="">-- Tất cả --</option>
