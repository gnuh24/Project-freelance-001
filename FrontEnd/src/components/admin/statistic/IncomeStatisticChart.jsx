...
const IncomeStatisticChart = () => {
    ...
    useEffect(() => {
        ...
        const getIncomeSummary = async () => {
            try {
                const response = await AxiosAdmin.get(`http://localhost:8080/Statistic/IncomeSummary?${query}`);
        ...
        <div className='space-y-10'>
            ...
            <div className='w-full'>
                {chartOptions.series.map((item) => {
                    ...
                    return (
                        <div key={item.name} className={`border w-full px-10 py-5 rounded-md cursor-pointer bg-[#e91e63]`}>
                            <h3 className='text-white text-center text-xl font-semibold'>Tổng thu nhập</h3>
                            <p className='text-white text-center text-md font-semibold '>{formatter.format(sum)} </p>
                        </div>
                    );
                })}
