...
const ShippingFeeTable = () => {
    ...
    const { data, status: apiStatus, error } = useSelector((state) => state.shippingFeeReducer || { data: { content: [], totalPages: 0 }, status: 'idle', error: null });

    const [pageNumber, setPageNumber] = useState(1);
    const pageSize = 5;
    const [sort, setSort] = useState('id,asc');
    const [itemDetails, setItemDetails] = useState(null);
    const [notification, setNotification] = useState(null); // Add state for notifications
                            ...
                            <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                                        {content.length === 0 ? (
                                            <tr>
                                                <td colSpan="5" className="px-4 py-4 text-sm text-gray-500 dark:text-gray-400 text-center">
