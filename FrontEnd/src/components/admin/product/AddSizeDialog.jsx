...
const AddSizeDialog = ({ isOpen, onClose, productId, setSizeSelected }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await AxiosAdmin.post(
        `${import.meta.env.VITE_API_URL}/ShoeSize/${productId}`,
        newForm,
      )
      const newSize = {
        size: data.size,
    ...
    <Dialog open={isOpen} onClose={onClose}>
      <DialogContent className="overflow-hidden">
        <div className="relative w-[35rem] space-y-2 overflow-hidden">
          ...
          <DialogTitle className="text-center">Thêm size mới</DialogTitle>
